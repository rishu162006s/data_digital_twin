...
_tick_now_event = threading.Event()  # set() to skip the rest of the current sleep and tick immediately
...
def live_loop():
    ...
    try:
        ...
        for timestamp, readings, _labels in simulate_stream(
            ...
            # Normally waits out the full TICK_SECONDS. But /scenario/bit-bad
            # and /scenario/worst call _tick_now_event.set() right after
            # seeding a fault, which cuts this wait short so the very next
            # tick (and therefore the new FAILURE-range prediction) lands in
            # well under a second -- not "whenever the next scheduled tick
            # happens to fall due", which could be up to TICK_SECONDS away.
            _tick_now_event.wait(timeout=TICK_SECONDS)
            _tick_now_event.clear()
...
def scenario_bit_bad():
    ...
    _tick_now_event.set()  # don't wait out the rest of TICK_SECONDS -- tick now
