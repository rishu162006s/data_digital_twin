...
def trigger_worst(fleet, trajectories, seed=None):
    ...
    hard_fail_assets = {aid for aid, _ in hard_fail}
    ...
    with _lock:
        ...
        for aid, sid in picked:
            ...
            if (aid, sid) in hard_fail:
                # Real, lasting failure -- joins the actively-degrading pool
                # (is_fault_asset() -> active=True in simulate_stream), so it
                # keeps trending toward failure tick after tick, same as the
                # "normal" scenario's real fault assets.
                _state["fault_assets"].add(aid)
                if aid in trajectories:
                    trajectories[aid].health = max(trajectories[aid].health, rng.uniform(0.85, 0.95))
            elif aid not in hard_fail_assets:
                # Only grazed: NOT added to fault_assets, so on the very next
                # tick AssetTrajectory.step(active=False) clamps its health
                # back down to <=0.12 on its own -- a one-tick spike that
                # self-heals without needing /admin/maintenance. dampened_assets
                # still smooths predicted_rul_days while it recovers so the
                # blip doesn't look like a crash.
                _state["dampened_assets"].add(aid)
                if aid in trajectories:
                    trajectories[aid].health = max(trajectories[aid].health, rng.uniform(0.3, 0.5))
            # else: this sensor is a "soft" one on an asset that ALSO has a
            # hard-failed sensor elsewhere in `picked` -- aid is already
            # (correctly) in fault_assets from the hard-fail branch above.
