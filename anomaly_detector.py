...
class AssetHealthMonitor:
    ...
    def update(self, zs):
        ...
        # time-based auto-fix, fires regardless of ANOMALY_AUTO_RECOVER
        ...
        if self.status == "SENSOR_FAILURE":
            status_message = (
                f"This sensor has failed and is running on backup "
                f"({anomaly_count} anomalies in the last {ANOMALY_WINDOW} readings). "
                f"Fixing automatically -- back to primary within {int(SENSOR_AUTO_FIX_SECONDS)}s."
            )
        else:
