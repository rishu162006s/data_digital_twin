...
def scenario_bit_bad():
    with _pipeline_lock:
        chosen = fault_scenario.trigger_bit_bad(_pipeline["fleet"], _pipeline["trajectories"])
...
def scenario_worst():
    with _pipeline_lock:
        result = fault_scenario.trigger_worst(_pipeline["fleet"], _pipeline["trajectories"])
    return {"triggered": "worst", **result, **fault_scenario.status()}
